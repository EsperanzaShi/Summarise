...
l2_lambda = 0.001  # Initial L2 penalty
collapse_triggered = False  # Track if collapse was detected in this epoch
for epoch in tqdm(range(EPOCHS), desc="Epochs"):
    ...
    for step, batch in enumerate(batch_iter):
        ...
        if not collapse_triggered and chosen_rewards.var() < 1e-6 and rejected_rewards.var() < 1e-6:
            ...
            # Lower learning rate by half for all param groups
            for param_group in optimizer.param_groups:
                param_group['lr'] = max(param_group['lr'] * 0.5, 1e-8)  # Don't go below 1e-8
            # Increase L2 penalty by 2x
            l2_lambda *= 2
            print(f"[INFO] New learning rate: {optimizer.param_groups[0]['lr']}, New L2 penalty: {l2_lambda}")
        ...
        for name, param in model.named_parameters():
            if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                print(f"[ERROR] Gradient for {name} is NaN or Inf! Stopping training.")
                nan_grad = True
                break
        if nan_grad:
            break
